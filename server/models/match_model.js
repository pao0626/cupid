...
const getCards = async (id, gender, pair ) => {
	...
	try {
		const [recordlist] = await conn.query('SELECT otherID FROM match_record WHERE userID = ?', [id]);
		...
		//判斷有沒有滑過其他人
		if(records.length > 0){
			[cards] = await conn.query('SELECT id, name, text, email, main_image, login_at FROM user WHERE gender = ? AND pair	= ? AND id NOT IN (?) LIMIT 20' , [pair, gender, records]);
		}
		else{
			[cards] = await conn.query('SELECT id, name, text, email, main_image, login_at FROM user WHERE gender = ? AND pair	= ? LIMIT 20' , [pair, gender]);
		}
		cards.map(
			card => {
